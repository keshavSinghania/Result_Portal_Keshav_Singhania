// UploadSessionalResult.jsx
import React, { useState } from "react";
import axios from "axios";
import { useSelector } from "react-redux";
import { getStudentsByYearAndSection, uploadSessionalResult } from "../../../axios";
import { useNavigate } from "react-router-dom";

const UploadSessionalResult = () => {
  ...
  const [sessionalNumber, setSessionalNumber] = useState(1);
  const [semesterType, setSemesterType] = useState("odd");
  const [message, setMessage] = useState("");
  const [messageColor, setMessageColor] = useState("green-800");
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate()

  const userDetails = useSelector((state) => state.auth.user);

  //fetching student according to the detail enntered by faculty
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await uploadSessionalResult(resultData);
      const data = res?.data;

      setMessage(data?.message || "Results uploaded successfully.");
    ...
    <div className="flex items-center justify-center py-10 px-4">
      <section className="bg-white w-full max-w-4xl rounded-lg shadow-md p-6">
        <div className="text-center mb-6">
          <h2 className="text-3xl font-bold text-purple-700">Upload Students Sessional Results</h2>
        ...
        {step === 3 && (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-600 mb-1">Sessional Number</label>
                <select
                  value={sessionalNumber}
                  onChange={(e) => setSessionalNumber(e.target.value)}
                  className="border border-gray-300 rounded p-2 w-full"
                >
        ...
        {step === 4 && (
          <div className="text-center mt-10 p-6 rounded-lg max-w-md mx-auto">
            ...
            <p className="text-green-700 text-base mb-6">
              All sessional marks have been uploaded successfully.
            </p>
            <button
              onClick={() => {
                navigate(-1);
